...
parser.add_argument('--loss_weights', nargs='+', type=float, default=[1,1,1,1,1,1,1,1], help='Loss weights')
...
parser.add_argument('--DIC_region', nargs='+', type=float, default=[0, 1, 0, 1], help='Region for DIC measurements in relative coordinates')
parser.add_argument('--measurments_type', choices=['displacement','strain'], default='strain', help='Type of measurements')
parser.add_argument('--num_measurments',nargs='+', type=int, default=[4, 4], help='Number of measurements (should be a perfect square)')
...
parser.add_argument('--params_iter_speed', nargs='+', type=float, default=1, help='Scale iteration step for each parameter')
parser.add_argument('--coord_normalization', type=int, default=0, help='Normalize the input coordinates')
parser.add_argument('--stress_integral', type=bool, default=False, help='Impose stress integral to be equal to the side load')
parser.add_argument('--stress_BC', type=bool, default=True, help='Impose stress BC on the right edge')
...
args.DIC_region = [bound*L_max for bound in args.DIC_region]
...
if material_law == "isotropic":
    ...
    params_actual = [E_actual, nu_actual]  # Actual parameters for optimization
    ...
    def constitutive_stress(eps_xx, eps_yy, eps_xy, mat_params=(E_init, nu_init)):
        ...
        E, nu = mat_params
        lmbd = E * nu / ((1 + nu) * (1 - 2 * nu)) # Plane strain Lamé parameter
        mu = E / (2 * (1 + nu)) 

        σ_xx = eps_xx * (lmbd + 2 * mu) + eps_yy * lmbd
        σ_yy = eps_yy * (lmbd + 2 * mu) + eps_xx * lmbd
        σ_xy = 2 * mu * eps_xy        
    ...
    E1_actual, E2_actual = 210e3, 105e3  # Actual Young's moduli 210 GPa and 105 GPa
    G12_actual, nu12_actual = 80e3, 0.3  # Actual shear modulus and Poisson's ratio
    params_actual = [E1_actual, E2_actual, G12_actual, nu12_actual]  # Actual parameters for optimization
    # Q11_init, Q22_init, Q12_init, Q66_init = 60e3, 5e3, 4e3, 2e3  # Initial guesses for orthotropic constants
    E1_init = E2_init = (E1_actual + E2_actual) / 2
    G12_init = 50e3
    nu12_init = 0.2  # Isotropic initial guess for Poisson's ratio


    params_init = [E1_init, E2_init, G12_init, nu12_init]  # Initial parameters for optimization
    params_names = ['E1', 'E2', 'G12', 'nu12']
    # theta = np.deg2rad(args.theta)  # Convert angle to radians for calculations

    def constitutive_stress(eps_xx, eps_yy, eps_xy, mat_params=(E1_init, E2_init, G12_init, nu12_init)):
        # plane‐stress modified constants
        E1, E2, G12, nu12 = mat_params
        nu21 = nu12 * E2 / E1  # Reciprocal relation
        Delta = 1 - nu12*nu21
        
        # Reduced stiffness matrix components for plane stress
        Q11 = E1 / Delta
        Q22 = E2 / Delta  
        Q12 = nu12 * E2 / Delta
        Q66 = G12
...
if not isinstance(args.params_iter_speed, list):
    args.params_iter_speed = [args.params_iter_speed] * len(params_init)
...
def integral_stress(x, outputs, X):
    x = transform_coords(x)
    # if args.coord_normalization:
    #     x *= jnp.array([L_max, L_max])  # denormalize coordinates
...
if args.measurments_type == "displacement":
    if args.DIC_dataset_path != "no_dataset":
        ...
        if args.num_measurments[0] * args.num_measurments[1] != x_values.shape[0] * y_values.shape[0]:
            print(f"For this DIC dataset, the number of measurements is fixed to {x_values.shape[0]} x {y_values.shape[0]}")
...
def HardBC(x, f, x_max=x_max):
    ...
    Sxx = f[:, 2] * (x_max - x[:, 0])/x_max + side_load(x[:, 1]) if args.stress_BC else f[:, 2]
...
batch_size = args.num_point_PDE + args.num_measurments[0]*args.num_measurments[1] + (n_integral if args.stress_integral else 0)
...
folder_name = f"{dic_prefix}{args.material_law}_{args.measurments_type}_{num_measurments_str}_{noise_prefix}_{args.available_time if args.available_time else args.n_iter}{'min' if args.available_time else 'iter'}"
...
bc_anchors = [integral_points] if args.stress_integral else []
bc_anchors += [X_DIC_input] * (len(bcs) - (1 if args.stress_integral else 0))
pde_anchors = [X_plot]
...
print(f"Material law: {material_law}")
print([f"{p_name}: {p_init * p.value * p_speed:.3f}" for p_name, p_init, p, p_speed in zip(params_names, params_init, params_factor, args.params_iter_speed)])
...
params_final = [np.mean([eval(line.strip().split(' ', 1)[1])[i] for line in lines[-10:]]) for i in range(len(params_init))]
print(f"Final parameters:" + ", ".join([f"{p_name}: {p_final:.3f}" for p_name, p_final in zip(params_names, params_final)]))
...
def log_config(fname):
    ...
    problem_info = {
        ...
        "stress_integral": args.stress_integral,
